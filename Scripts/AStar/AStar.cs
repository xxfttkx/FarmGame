...
namespace MFarm.AStar
{
    ...
    {
        private GridNodes gridNodes;//当前地图
        private Node startNode;     //起始网格
        private Node targetNode;    //目标网格
        private int gridWidth;      //地图的宽度
        private int gridHeight;     //地图的高度
        private int originX;        //原点坐标的X值
        private int originY;        //原点坐标的Y值
        private List<Node> openNodeList;    //仍待探索的节点
        private HashSet<Node> closedNodeList;   //已经探索过的节点
        private bool pathFound;     //是否已找到从起点到终点的路径
                ...
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    if (validNeighbourNode != null)
                    {
                        if (!openNodeList.Contains(validNeighbourNode))
                        {
                            ...
                            validNeighbourNode.hCost = 10 *
                                (Mathf.Abs(targetNode.gridPosition.x - validNeighbourNode.gridPosition.x) +
                                Mathf.Abs(targetNode.gridPosition.y - validNeighbourNode.gridPosition.y));
                            //链接父节点
                        ...
                        {
                            var newGCost = currentNode.gCost + GetDistance(currentNode, validNeighbourNode);
                            if (newGCost < validNeighbourNode.gCost)
                            {
                                validNeighbourNode.gCost = newGCost;
                                validNeighbourNode.parentNode = currentNode;
                            }
                        }
                    }
        ...
        private void UpdatePathOnMovementStepStack(string sceneName, Stack<MovementStep> npcMovementStep)
        {
            ...
            while (nextNode != null)
            {
                ...
                newStep.gridCoordinate = 
                    new Vector2Int(nextNode.gridPosition.x + originX, 
                        nextNode.gridPosition.y + originY);
                //压入栈中
