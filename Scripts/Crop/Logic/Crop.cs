...
public class Crop : MonoBehaviour
{
    ...
    /// </summary>
    /// <returns></returns>
    public bool CanHarvestToBag()
    {
        int emptySlotsCount = InventoryManager.Instance.CountEmptySlots();
        for (int i = 0; i < cropDetails.producedItemID.Length; i++)
        {
            int itemID = cropDetails.producedItemID[i];
    ...
    void Init()
    {
        cropDetails = CropManager.Instance.GetCropDetails(cropDetails.seedItemID);
        if (cropDetails == null) return;
        if (cropDetails.harvestToolItemID.Length == 1
            && cropDetails.harvestToolItemID[0] == 1004)
        {
            if (gameObject.GetComponent<ReapItem>() == null)
                gameObject.AddComponent<ReapItem>();
            gameObject.GetComponent<ReapItem>().InitCropData(cropDetails.seedItemID);
            if (gameObject.GetComponent<ItemInteractive>() == null)
