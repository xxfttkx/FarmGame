...
public class CursorManager : MonoBehaviour
{
    ...
    private Vector3 mousePos => Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, 0));
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(1))
        {
            ...
            {
                var colls = GetMousePositionColliders();
                foreach (var coll in colls)
                {
                    var box = coll.gameObject.GetComponent<Box>();
                    if (box != null)
                    {
                        box.OpenBox();
                    }
    ...
    Collider2D[] GetMousePositionColliders()
    {
        return Physics2D.OverlapPointAll(mousePos);
    }
    private void CheckPlayerInput()
    {
        if (Input.GetMouseButton(0) && cursorPositionValid)
        {
            //执行方法
    ...
    private void OnItemSelectedEvent(ItemDetails itemDetails, bool isSelected)
    {
        ...
        else    //物品被选中才切换图片
        {
            ...
            if (itemDetails.itemType == ItemType.Furniture)
            {
                ...
                // float w = buildImage.sprite.rect.width;
                // float h = buildImage.sprite.rect.height;
                float w = Settings.pixelsPerUnit;
                float h = buildImage.sprite.rect.height / buildImage.sprite.rect.width * w;
                buildImage.rectTransform.sizeDelta = new Vector2(w, h);
                buildImage.SetAllDirty();
    ...
    private void CheckCursorValid()
    {
                ...
                case ItemType.CollectTool:
                    if (currentCrop != null)
                    {
                        if (currentCrop.CheckToolAvailable(currentItem.itemID))
                            if (currentTile.growthDays >= currentCrop.TotalGrowthDays && crop.CanHarvestToBag())
                                SetCursorValid(); 
                            else 
                                SetCursorInValid();
                    }
        ...
        {
            if (currentItem.itemType == ItemType.ReturnTool)
                SetCursorValid();
            else SetCursorInValid();
        }
    }
