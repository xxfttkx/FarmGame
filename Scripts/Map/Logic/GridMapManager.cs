...
namespace MFarm.Map
{
    public class GridMapManager : Singleton<GridMapManager>, ISaveable
    {
        ...
        private void OnAfterSceneLoadedEvent()
        {
            ...
            if (firstLoadDict.ContainsKey(SceneManager.GetActiveScene().name))
            {
                //预先生成农作物
        ...
        private void OnGameDayEvent(int day, Season season)
        {
            ...
            if (tileDetailsDict.Count > 0)
            {
                foreach (var tile in tileDetailsDict)
                {
                    //随机生成杂草、树、石头
                    string key = tile.Key;
                    if(key.Contains("01.Field"))
                    {
                        var tileDetails = tile.Value;
                        if(tileDetails.canDropItem) //todo:改成别的属性
                        {
                            float flag = Random.Range(0f,1f);
                            if (flag < Settings.dailyGenerateGrass + Settings.dailyGenerateStone
                                && tileDetails.seedItemID == -1)
                            {
                                int ID = -1;
                                if (flag < Settings.dailyGenerateGrass)
                                {
                                    //杂草
                                    ID = Random.Range(1024,1027);
                                }
                                else if (flag < Settings.dailyGenerateGrass + Settings.dailyGenerateStone)
                                {
                                    //石头
                                    ID = Random.Range(1021,1023);
                                    tileDetails.isNPCObstacle = true;
                                }
                                else
                                {//todo:生成树
                                    //ID = Random.Range(0f, 1f) > 0.5f ? 1016 : 1018;
                                }
                                EventHandler.CallPlantSeedEvent(ID, tileDetails);
                            }
                    ...
                    else
                    {
                        ...
                        //超期消除挖坑
                        if (tile.Value.daysSinceDug > 3 && Random.Range(0f, 1f) < 0.3f)
                        {
        ...
        public TileDetails GetTileDetailsByPosAndSceneName(Vector3Int GridPos, string sceneName)
        {
            string key = GridPos.x + "x" + GridPos.y + "y" + sceneName;
            return GetTileDetails(key);
        }

        public TileDetails GetTileDetailsByPosAndSceneName(Vector2Int GridPos, string sceneName)
        {
            string key = GridPos.x + "x" + GridPos.y + "y" + sceneName;
            return GetTileDetails(key);
        }
        ...
        private void OnExecuteActionAfterAnimation(Vector3 mouseWorldPos, ItemDetails itemDetails)
        {
            ...
            if (currentTile != null)
            {
                ...
                switch (itemDetails.itemType)
                {
                    ...
                    case ItemType.ChopTool:
                        ...
                        currentTile.isNPCObstacle = false;
                        break;
                    ...
                    case ItemType.Furniture:
                        ...
                        break;
                    case ItemType.ReturnTool:
                        EventHandler.CallTransitionEvent(Settings.playerStartScene, Settings.playerStartPos);
                        Player.Instance.SetToScreen();
                        break;
                    default:
                        break;
            ...
            {
                //ReturnTool不考虑currTile  要不要写在transitionManager中
                if (itemDetails.itemType == ItemType.ReturnTool)
                {
                    EventHandler.CallTransitionEvent(Settings.playerStartScene, Settings.playerStartPos);
                    Player.Instance.SetToScreen();
                }
        ...
        private void RefreshMap()
        {
            ...
            var crops = FindObjectsOfType<Crop>();
